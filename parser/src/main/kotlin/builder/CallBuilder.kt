...
class CallBuilder: ASTBuilderStrategy {
    override fun build(token: Token, tokens: List<Token>): ASTNode {
        ...
        when(tokens[tokenPos+ 2].getType()){
            TokenType.LITERAL_NUMBER ->args.add( LiteralNode(tokens[tokenPos+ 2], 0,0))
            TokenType.LITERAL_STRING ->args.add(LiteralNode(tokens[tokenPos + 2], 0,0))
            TokenType.IDENTIFIER -> args.add(IdentifierNode(tokens[tokenPos + 2], 0,0))
            else -> error("Unexpected token type")
        }
        return CallNode(token, args, 0,0)
    }
